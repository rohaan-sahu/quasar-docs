...
pub struct MyInstruction<'info> {
    #[account(mut, signer, has_one = authority)]
...
Requires the account to be a transaction signer. Automatically implied for `Signer` type fields; use this on other types when signer status is needed.

| | |
|---|---|
| **Syntax** | `#[account(signer)]` |
| **Error on failure** | `ProgramError::MissingRequiredSignature` |

```rust
#[account(mut, signer)]
pub authority: &'info mut Account<Vault>,
```
...
These attributes validate SPL token account fields.

### `token::mint = <field>`

Validates that the token account's mint matches the address of the referenced mint account.

| | |
|---|---|
| **Syntax** | `#[account(token::mint = mint_field)]` |
| **Error on failure** | `ConstraintViolation` |

### `token::authority = <field>`

Validates that the token account's authority matches the address of the referenced account.

| | |
|---|---|
| **Syntax** | `#[account(token::authority = authority_field)]` |
| **Error on failure** | `ConstraintViolation` |
...
| `signer` | `#[account(signer)]` | `MissingRequiredSignature` |
| `dup` | `#[account(dup)]` | -- |
...
| `token::mint` | `#[account(token::mint = field)]` | `ConstraintViolation` |
| `token::authority` | `#[account(token::authority = field)]` | `ConstraintViolation` |
